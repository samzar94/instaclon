...
async function isFollow(username,ctx) {
    const userFound= await User.findOne({username});
    if(!userFound) throw new Error("usuario no encontrado");
    const follow= await Follow.find({idUser:ctx.user.id}).where("follow").equals(userFound._id);//para comprobar si estoy siguiendo al usuario
    if(follow.length>0){
        return true;
    }
    else return false;
